...
class Attorney(MyWin):
    ...
    def initialite(self):
        ...
        self.search.actions()[2].triggered.connect(lambda :os.startfile("https://cn.bing.com/search?q="+self.search.text()))
        for i in range(6):
        ...
        def showlog():
            ...
            if log[-1] is None:
                ...
                assi = re.sub("\n[0,1]*","",log[-1])
            self.show_log.setPlainText("计划日："+str(log[1])+"\n结束日："+str(log[5])+"\n办理人："+str(assi)+"\n主题："+str(log[3])+"\n内容："+str(log[4]))
        ...
        who = [self.show_law_txt, self.search, self.log_name.lineEdit(), self.collect_name, self.log_case.lineEdit(),self.link_title,self.link_link, self.show_log,
    ...
    def creatcalendar(self):
        ...
        fetchall = self.sendsql(["select 日志.*,案件.id from 日志 left outer join 案件 on 日志.caseid=案件.id where 日志.startdate like '%{}%'{} group by 日志.id order by 日志.startdate asc".format(str(self.calendar.ys.value())+"-"+self.calendar.mc.currentData(),sql)])
        if fetchall == 0:
            fetchall = ()
        for day in range(1, wl[1] + 1):
            ...
            for data in fetchall:
                if self.abort:
                    return
                if (self.calendar.bg.checkedId() == 2 and data[-1] is None) or (self.calendar.bg.checkedId() == 3 and data[-1] is not None):
                    continue
    ...
    def finishsetting(self,result):
        ...
        if result == 2:
            ...
            if admin == "1":
                ...
                if len(name) > 0 and name[0][0] != self.me["id"]:
                    self.do_something_sig.emit([self.msg,["<font color='#DB3022'><b>{}</font><br>".format(name[0][1].split("\n")[0])+"已经是管理员！"]])
                    admin = "0"
        ...
        if w == 1:
            ...
            self.sendsql(["insert or replace into 用户 values(?,?,?)"],(self.me["id"],self.me["姓名"],self.me["头像"]))
            with open("profile.bt", "wb") as t:
                t.write(pickle.dumps(self.me))
            try:
                os.startfile("getnewlaws.exe")
    ...
    def output_paper(self):
        def getsql(table):
            ...
            return "select {}.*,用户.name from {} left outer join 用户 on {}.assistant=用户.id{} group by {}.id order by {} desc".format(table,table, table,sql, table,table+"."+order)
    ...
    def fill_case_name(self):
        def fill():
            ...
            if self.log_case.currentText() == "":
                ...
                return
            fetchall = self.sendsql(["select * from 案件 where casename like '%{}%'".format(self.log_case.currentText()),"select * from 法务 where name like '%{}%'".format(self.log_case.currentText())])
            if fetchall == 0:
                fetchall = ()
            many = len(fetchall)
            if many > 0:
                ...
                while self.log_case.count() > 0:
                    time.sleep(0.01)
                for i in fetchall:
                    self.do_something_sig.emit([self.log_case.addItem,i[6 if len(i) != 12 else 5]+" "+i[1],list(i)])
    ...
    def copyfiles(self,what):
        ...
        for i in what[2]:
            ...
            try:
                ...
                no.append(name)
        if len(no) != 0:
            self.do_something_sig.emit([self.msg,["<font color='#DB3022'>下列文件提交失败：</font><br>" + "<br>".join(no)]])
    ...
    def write_log_case_client_collect(self,table=None,record=False):
        ...
        if table == "案件":
            ...
            self.current_case[6:9] = [self.case_start.date().toString("yyyy-MM-dd"),self.case_end.date().toString("yyyy-MM-dd") if self.case_end.isEnabled() else "",assitant]
            data = self.current_case[0:9]+[pickle.dumps(i) for i in self.current_case[9:]]
        if table == "日志":
            self.current_log = [self.current_log[0],self.log_start.date().toString("yyyy-MM-dd")+self.log_start_time.time().toString(" HH:mm"),self.working_data[0],
                                self.log_name.currentText()+"."+str(self.log_count.value()), self.log_what.toPlainText(),
                               self.log_end.date().toString("yyyy-MM-dd") if self.log_end.isEnabled() else "",assitant,self.current_log[-1]]
            if record:
        ...
        if table == "法务":
            if record:
                ...
                if self.current_client[1] != "" and self.current_client[1]!= txt and os.path.exists(dirs + self.current_client[1]):
            ...
            self.current_client = [self.current_client[0], *self.current_client[1:5],self.client_case_start.date().toString("yyyy-MM-dd"),self.client_case_end.date().toString("yyyy-MM-dd"),*self.current_client[7:9],assitant,*self.current_client[-2:]]
        ...
        if not record or record == "右键收藏":
            ...
            if msg != "":
                ...
                def ok(table,index,date):
                    ...
                    self.prepare(index)
                    if table == "日志" and self.stack.currentIndex() == 0 and str(self.calendar.ys.value())+"-"+self.calendar.mc.currentData() in date:
                        self.setdates()
    ...
    def tree_menu(self):
            ...
            if self.sender().text() == u'删除':
                ...
                if yn == QMessageBox.Ok:
                    if self.main_tree.currentItem() in self.law_items[0]+self.law_items[1:]:
                        ...
                        self.record_database(["delete from {} where title='{}'".format(table_data[0],title)],db="LawData.db")
                        self.record_database(["delete from {} where title='{}'".format("常用法律",title)],db="LawData.db")
                    ...
                    if self.main_tree.currentItem() in self.current_items.values():
                        def delitem(table_data):
                            fetchall = self.sendsql(["delete from {} where id='{}'".format(table_data[0],table_data[1][0])])
                            if fetchall == 0:
                                return
                            self.do_something_sig.emit([self.main_tree.takeTopLevelItem,self.main_tree.currentIndex().row()])
                            del self.current_items[table_data[1][0]]
                            if table_data[0] == "日志" and self.stack.currentIndex() == 0 and str(self.calendar.ys.value())+"-"+self.calendar.mc.currentData() in table_data[1][1]:
                ...
                if index == -1:
                    return
                self.record_database(["insert or replace into 常用法律 (title) values(?)"], (table_data[1],),db="LawData.db")
                ...
                self.record_database(["delete from {} where title='{}'".format("常用法律",table_data[1])],db="LawData.db")
                ...
                thread = Thread(self.sendsql,["update 日志 set color= '{}' where id='{}'".format(color,table_data[1][0])])
                thread.setAutoDelete(True)
                self.thread_pool.start(thread)
        ...
        if txt != "":
            ...
            groupBox_menu.addAction(actionA)
        if self.main_tree.currentItem() in self.current_items.values() or self.main_tree.currentItem() in self.law_items[0] + self.law_items[1:]:
    ...
    def set_item(self,data,table="",w=-1):
        ...
        def setroot(root,icon,txt,color):
        ...
        self.do_something_sig.emit([partial(setroot,root,icon,txt,color)])
        time.sleep(0.01)
    ...
    def load_alllaws(self):
        ...
        def sss(x):
            ...
            return st
        for table in tables:
            all_laws_title[table[0]] = self.record_database(["select title,name from {}".format(table[0])],db="LawData.db")
        ...
        for table in all_laws_title.keys():
            ...
            for title_name in all_laws_title[table]:
                ...
                index = 0
                parent = root
                self.all_laws[table][title_name[1]] = (title_name[0],pickle.loads(self.record_database(["select * from {} where name='{}'".format(table,title_name[1])],db="LawData.db")[0][2]))
    ...
    def localhost_func(self):
        while self.localhost_udp.hasPendingDatagrams():
            ...
            if os.path.exists("gongbao.db"):
                ...
                index = 0
                for table in self.record_database(["select name from sqlite_master where type='table'"],db="gongbao.db"):
                    table = table[0]
    ...
    def show_datas_all(self, table, start=0):
        if start == 0:
            ...
            if table == "法务":
                ...
                order = "startdate"
            if table == "法务":
                word = "select 法务.* ,max(日志.startdate) from 法务 left outer join 日志 on 法务.id=日志.caseid{} group by 法务.id order by {} desc".format(sql, table+"."+order)
            # elif table == "日志":
    ...
    def append_law(self):
        ...
        for code in current_law[index:]:
            if code is None:
                continue
            if zjb != "" and (re.search("^第.+?" + zjb + "\s+", code) is not None or re.search("^" + zjb + "+[\W\s]*?", code) is not None):#找到下一个zjb
    ...
    def show_log_a(self):
        ...
        def log_case(current_log):
            ...
            fetchall = self.sendsql(["select * from 案件 where id='{}'".format(current_log[2]),"select * from 法务 where id='{}'".format(current_log[2])])
            if fetchall == 0:
                return
            self.do_something_sig.emit([self.log_case.clear])
            while self.log_case.count() > 0:
            ...
            if len(fetchall) == 0:
                ...
                self.do_something_sig.emit([self.log_case.addItem,self.current_client[5]+" "+self.current_client[1], self.current_client])
            else:
                self.current_case = list(fetchall[0])
                self.do_something_sig.emit([self.log_case.addItem,self.current_case[6]+" "+self.current_case[1], self.current_case])
    ...
    def show_linkwin(self):
        ...
        def dellink(r,c):
            ...
            self.link_box.removeItem(item)
            item.widget().deleteLater()
            self.record_database(["delete from links where title='{}' and link='{}'".format(self.sender().data()[0],self.sender().data()[1])],db="links.db")
        ...
        for i in fetchall:
            ...
            link_button.setStyleSheet("QToolButton:hover {background-color:rgba(72, 175, 255,100);border:0px;border-radius:6px;} QToolButton {background-color:transparent;border:0px;}")
    ...
    def add_new(self,table=""):
        ...
        def add_log():
            ...
            if len(editting) != 0:
                ...
                self.current_log = [str(time.strftime("%Y%m%d%H%M%S")),str(time.strftime("%Y-%m-%d")),"" if len(self.working_data) == 6 else self.working_data[0],"","","","#000000"]
            self.show_log_a()
        ...
        def add_law():
            ...
            if len(lawfiles) != 0:
                ...
                tbl.setComboBoxItems(["法律","宪法","司法解释","行政法规","地方性法规","部门规章","地方政府规章","规范性文件"])
                if tbl.exec_() == 1:
                    table = tbl.textValue()
                else:
                    return
    ...
    def show_client_a(self):
        ...
        if self.current_client[0] == "":
            self.current_client[0] = str(self.client_case_start.date().toString("yyyyMMdd")) + str(time.strftime("%H%M%S"))
        ...
        labels = ["","名称", "行业", "联系电话", "联系地址","","", "套餐", "回款/签约金额", "","签约顾问","备注","服务剩余"]
        for i in labels:
            if i == "":
                continue
    ...
    def show_havedone(self,current,fetchall=0):
        ...
        while self.have_done.topLevelItemCount() > 0:
            time.sleep(0.01)
        if fetchall == 0:
            fetchall = self.sendsql(["select 日志.*,用户.name from 日志 left outer join 用户 on 日志.assistant=用户.id where 日志.caseid='{}' order by 日志.startdate desc".format(current[0])])
            if fetchall  == 0:
                fetchall = ()
        if len(fetchall) == 0:
        ...
        def addroot(root):
            ...
            self.have_done.setItemWidget(root,0,self.log_files.createwidget([""],partial(self.deletelog,root),"delete.png"))
    ...
    def show_case_a(self):
        ...
        for i in labels:
            ...
            if labels.index(i) in [3,4,7,8]:
                continue
            self.tree_case.setItemWidget(root, 0, self.log_files.createwidget(["加"], partial(self.case_creat_line,root),"add.png"))
    ...
    def client_creat_line(self,root,index):
        ...
        def countservice(label,current_client):
            ...
            fetchall = self.sendsql(["select 日志.*,用户.name from 日志 left outer join 用户 on 日志.assistant=用户.id where 日志.caseid='{}' group by 日志.id order by 日志.startdate desc".format(current_client[0])])
            ...
            for i in meal.keys():
                if type(last[i]) is int and last[i] <= 1:
                    txt = txt + "<font color = #000000 size=4>{}：</font><font color = #ff0000 size=4>{}/{}</font><br>".format(i,str(last[i]),str(meal[i]))
                else:
    ...
    def case_creat_line(self,root):
        ...
        if self.sender().objectName() == "加":
            ...
            ran = [[self.current_case[dataindex],"",""]] if type(self.current_case[dataindex]) is str else self.current_case[dataindex]
            n = 0
        for i in ran:
            if len(i) < 3:
                i = i + ["",""]
    ...
    def more_inf(self,dataindex,n):
        ...
        self.more_inf_txt.move(self.x()+(self.width()-self.more_inf_txt.width())//2,self.y()+(self.height()-self.more_inf_txt.height())//2)
    ...
    def find_in_all(self):
        ...
        for i in range(6):
            if self.search_which.itemAt(i).widget().isChecked():
                scope.append(["日志","案件","法务","收藏","法律","文件"][i])
            ...
            for data in datas:
                ...
                for k in data[1:]:
                    ...
                    if which == "案件":
                        ...
                        if m == 7:
                            a = self.record_database(["select name from 用户 where id='{}'".format(str(k))],db="database.db")
                            if a != 0 and len(a) != 0:
                                k = a[0][0]
                            else:
                                k = ""
                    ...
                    if which == "日志":
                        if m == 1:
                            name = self.record_database(["select casename from 案件 where id='{}'".format(str(k)),"select name from 法务 where id='{}'".format(str(k))],db="database.db")
                            if name == 0:
                                name = ()
                            if len(name) != 0:
                                k = name[0][0]
