...
def convert_to_bytes(table,laws):
    ...
    cur = conn.cursor()
    cur.execute('''create table if not exists {} (title TEXT PRIMARY KEY,codes BLOB)'''.format(table))
    conn.commit()
    ...
    for i in laws.keys():
        ...
        for code in codes:
            if re.search("(^第.+?[条节章编][、.\s]*?)|(^[0-9一二三四五六七八九十]+[、.\s]*?)", code) is not None:
                ...
                law[-1] = law[-1] + code
        law = law[1:]
        cur.execute("insert or replace into {} (title,codes) values(?,?)".format(table), (title,pickle.dumps(law)))
    conn.commit()
    conn.close()
...
def record_law(table,files):
    ...
    for i in files:
        ...
        ok = kk = 0
        zjp = []
        txt = f_e[0].split("\\")[-1].replace("中华人民共和国","").replace("人民代表大会","人大").replace("常务委员会","常委")
